...
Shader "Shader Forge/RootRunes" {
    Properties {
        ...
        _UpNode_copy ("UpNode_copy", Range(-30, 1)) = 0.975541
...
////// Specular:
                ...
                float3 diffuseColor = lerp((_MainTex_var.rgb*_Color.rgb),(_MudDiffuse_copy_var.rgb*_Mud_Color.rgb),node_5950); // Need this for specular when using metallic
...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float node_270 = 0.0;
                float node_5950 = saturate(pow((max(0,dot(float3(node_270,_UpNode_copy,node_270),i.posWorld.rgb))*_Level_copy),_Contrast_copy));
                float3 diffColor = lerp((_MainTex_var.rgb*_Color.rgb),(_MudDiffuse_copy_var.rgb*_Mud_Color.rgb),node_5950);
                float specularMonochrome;
                float3 specColor;
